                                        ...
                                        <div id="return-search-results">
                                            <div class="table-responsive">
                                                ...
                                                <div class="row">
                                                    <div class="col-sm-3">
                                                        <div class="mb-3">
                                                            <label class="form-label" for="return_price_adult">Adult
                                                                Publish
                                                                (IDR)</label>
                                                            <input type="number" class="form-control"
                                                                id="return_price_adult" name="return_price_adult"
                                                                oninput="calculateReturnTotal()">
                                                        </div>
                                                    </div>
                                                    <div class="col-sm-3">
                                                        <div class="mb-3">
                                                            <label class="form-label" for="return_price_child">Child
                                                                Publish
                                                                (IDR)</label>
                                                            <input type="number" class="form-control"
                                                                id="return_price_child" name="return_price_child"
                                                                oninput="calculateReturnTotal()">
                                                        </div>
                                                    </div>
                                                    <div class="col-sm-3">
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#tripReturnCheckbox').change(function() {
                if ($(this).is(':checked')) {
                    ...
                    $('#tripReturnForm').hide();
                    $('#tripReturnForm').find('input, select').prop('disabled', true).val(
                        ''); // Clear form inputs
            ...
            $('#fbo_trip_date_return').change(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        populateDropdown('#fbo_departure_port_return', response
                            .fbo_departure_ports_return, 'Select Departure Port');
                    },
                    error: function() {
            ...
            $('#fbo_departure_port_return').change(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        populateDropdown('#fbo_arrival_port_return', response
                            .fbo_arrival_ports_return, 'Select Arrival Port');
                    },
                    error: function() {
            ...
            $('#fbo_arrival_port_return').change(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        populateDropdown('#fbo_departure_time_return', response
                            .fbo_departure_times_return, 'Select Time Return');
                    },
                    error: function() {
        ...
        $(document).ready(function() {
            // Ketika tab berpindah
            $('a[data-bs-toggle="tab"]').on('shown.bs.tab', function(e) {
                // Mengambil id dari tab yang aktif
                var activeTab = $(e.target).attr('href').substring(1); // Ambil ID tab yang aktif

                // Update nilai input tersembunyi pada form
                $('input[name="active_tab"]').val(activeTab);
            });

            // Set nilai awal tab aktif saat halaman pertama kali dimuat
            var activeTab = $('.nav-pills .nav-link.active').attr('href').substring(1); // Ambil ID tab yang aktif
            $('input[name="active_tab"]').val(activeTab);
        });
    </script>
@endsection
