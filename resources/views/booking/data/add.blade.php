...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#trip_date, #departure_port, #arrival_port, #fast_boat, #time_dept').on('change', function() {
            resetSearchResults(); // Sembunyikan hasil pencarian sebelum pencarian ulang
        ...
        function performSearch(tripDate, departurePort, arrivalPort, fastBoat, timeDept) {
            ...
            function updateCurrencyLabel() {
                ...
                let currencyCode = selectedOption.data('code') ||
                    'IDR'; // Ambil kode mata uang atau default ke IDR
                // Ubah teks label
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.html) {
                        ...
                        // Jika tidak ada hasil, sembunyikan hasil
                        resetSearchResults();
                    }
...
<script>
    $(document).ready(function() {
        ...
        // Ketika jumlah dewasa atau anak diubah, sembunyikan hasil pencarian
        $('#adult_count, #child_count').on('change', function() {
            resetSearchResultsReturn();
        });
        ...
        function getCustomerCountReturn() {
            return {
                adult_count: $('#adult_count').val() || 1,
                child_count: $('#child_count').val() || 0
            };
        }

        // Fungsi untuk reset dropdown
        function resetDropdowns() {
            $('#departure_return_port, #arrival_return_port, #fast_boat_return, #time_dept_return').empty().append('<option value="">Select</option>');
        }
        ...
        $('#departure_return_port').change(function() {
            ...
            $('#fast_boat_return, #time_dept_return').empty().append('<option value="">Select</option>');
